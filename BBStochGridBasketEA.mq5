...
bool RefreshBarData()
  {
   double st[1], up[1], lo[1];
   // iBands: buffer 1 = upper band, buffer 2 = lower band (buffer 0 = middle)
   if(CopyBuffer(g_hStoch, 0, 0, 1, st) != 1)
     {
      Print("BBStochGridBasketEA: CopyBuffer Stochastic main failed. err=", GetLastError());
      return(false);
     }
...
bool CheckSellSignal()
  {
   if(g_stochMain <= (double)StochasticOverboughtLevel)
      return(false);
   ...
   if(touchOrCross)
      Print("ENTRY SIGNAL: SELL setup — StochMain=", DoubleToString(g_stochMain, 2),
...
bool CheckBuySignal()
  {
   if(g_stochMain >= (double)StochasticOversoldLevel)
      return(false);
   ...
   if(touchOrCross)
      Print("ENTRY SIGNAL: BUY setup — StochMain=", DoubleToString(g_stochMain, 2),
