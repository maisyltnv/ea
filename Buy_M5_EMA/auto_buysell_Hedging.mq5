...
//| ຫຼັງ TP: Auto BUY ຄັ້ງຕໍ່ໄປຕ້ອງ M1 Close>EMA14>26>50>100>200 ກ່ອນເປີດ |
//+------------------------------------------------------------------+
...
#property description "Default: XAUUSD chart only (InpOnlyXAUUSD)."
#property version "1.02"

#include <Trade/Trade.mqh>

//--- M1 EMA filter: ຫຼັງ TP ກ່ອນ Auto BUY ຕ້ອງ close > EMA14>26>50>100>200
...
input int TPPoints = 400;       // Take profit distance (points) from each entry
...
input bool   InpOnlyXAUUSD = true; // true = ໃຊ້ແຜງ XAUUSD ເທົ່ານັ້ນ (ຮອງຮັບ XAUUSDm ຯລ)
...
bool     g_needM1FilterForAutoBuy =
    false; // true ຫຼັງໂດນ TP ແລ້ວ — ຄັ້ງຕໍ່ໄປຖ້າ Auto Buy ຕ້ອງຜ່ານ EMA M1

int g_ema14_m1  = INVALID_HANDLE;
int g_ema26_m1  = INVALID_HANDLE;
int g_ema50_m1  = INVALID_HANDLE;
int g_ema100_m1 = INVALID_HANDLE;
int g_ema200_m1 = INVALID_HANDLE;
datetime g_lastAutoBuyEmaLogBar = 0;

//+------------------------------------------------------------------+
//| ສັນຍານແຜງຕ້ອງເປັນ XAUUSD (ຂຶ້ນຕົ້ນດ້ວຍ XAUUSD — ຮອງຮັບ suffix broker) |
//+------------------------------------------------------------------+
bool ChartSymbolIsXAUUSD() {
  string s = _Symbol;
  StringToUpper(s);
...
//+------------------------------------------------------------------+
//| M1: ລາຄາປິດ > EMA14 > EMA26 > EMA50 > EMA100 > EMA200 (ແທ່ງປິດລ່າສຸດ) |
//+------------------------------------------------------------------+
bool IsM1BullishEMAStack() {
  const int sh = 1;
  double close = iClose(_Symbol, PERIOD_M1, sh);
  if (close <= 0.0)
    return false;

  double e14[], e26[], e50[], e100[], e200[];
  if (CopyBuffer(g_ema14_m1, 0, sh, 1, e14) != 1)
    return false;
  if (CopyBuffer(g_ema26_m1, 0, sh, 1, e26) != 1)
    return false;
  if (CopyBuffer(g_ema50_m1, 0, sh, 1, e50) != 1)
    return false;
  if (CopyBuffer(g_ema100_m1, 0, sh, 1, e100) != 1)
    return false;
  if (CopyBuffer(g_ema200_m1, 0, sh, 1, e200) != 1)
    return false;

  return (close > e14[0] && e14[0] > e26[0] && e26[0] > e50[0] &&
          e50[0] > e100[0] && e100[0] > e200[0]);
}
...
//+------------------------------------------------------------------+
int OnInit() {
  if (InpOnlyXAUUSD && !ChartSymbolIsXAUUSD()) {
    Print("EA ສຳລັບ XAUUSD ເທົ່ານັ້ນ — ສັນຍານແຜງ: ", _Symbol,
          " (ປິດ InpOnlyXAUUSD=false ຖ້າທົດສອບສັນຍານອື່ນ)");
    return (INIT_FAILED);
  }
  ...
  g_ema14_m1 = iMA(_Symbol, PERIOD_M1, 14, 0, MODE_EMA, PRICE_CLOSE);
  g_ema26_m1 = iMA(_Symbol, PERIOD_M1, 26, 0, MODE_EMA, PRICE_CLOSE);
  g_ema50_m1 = iMA(_Symbol, PERIOD_M1, 50, 0, MODE_EMA, PRICE_CLOSE);
  g_ema100_m1 = iMA(_Symbol, PERIOD_M1, 100, 0, MODE_EMA, PRICE_CLOSE);
  g_ema200_m1 = iMA(_Symbol, PERIOD_M1, 200, 0, MODE_EMA, PRICE_CLOSE);

  if (g_ema14_m1 == INVALID_HANDLE || g_ema26_m1 == INVALID_HANDLE ||
      g_ema50_m1 == INVALID_HANDLE || g_ema100_m1 == INVALID_HANDLE ||
      g_ema200_m1 == INVALID_HANDLE) {
    Print("Failed to create M1 EMA handles");
    return (INIT_FAILED);
  }

  Print("AutoBuySellHedgingManual initialized on symbol ", _Symbol,
        " | auto cycles starting from BUY.");
  Print("M1 filter for Auto BUY: close>EMA14>26>50>100>200 (last closed bar).");
...
void OnDeinit(const int reason) {
  DeleteStartButtons();
  if (g_ema14_m1 != INVALID_HANDLE)
    IndicatorRelease(g_ema14_m1);
  if (g_ema26_m1 != INVALID_HANDLE)
    IndicatorRelease(g_ema26_m1);
  if (g_ema50_m1 != INVALID_HANDLE)
    IndicatorRelease(g_ema50_m1);
  if (g_ema100_m1 != INVALID_HANDLE)
    IndicatorRelease(g_ema100_m1);
  ...
  if (g_mode == MODE_NONE) {
    ...
    if (hasManualBuy && !hasManualSell) {
      ...
      if (g_nextMode == MODE_BUY_START) {
        if (g_needM1FilterForAutoBuy && !IsM1BullishEMAStack()) {
          datetime m1bar = iTime(symbol, PERIOD_M1, 0);
          if (m1bar != 0 && m1bar != g_lastAutoBuyEmaLogBar) {
            g_lastAutoBuyEmaLogBar = m1bar;
            Print("[Auto BUY ຫຼັງ TP] ລໍຖ້າ M1: Close>EMA14>EMA26>EMA50>EMA100>EMA200 "
                  "(ແທ່ງປິດລ່າສຸດ) — ຍັງບໍ່ເປີດອໍເດີ");
          }
        } else if (trade.Buy(lot, symbol, ask, 0.0, 0.0, "AutoStart BUY")) {
          g_needM1FilterForAutoBuy = false;
