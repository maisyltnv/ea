...
bool PlaceInitialMarket(StartSide side) {
  ...
  bool ok = false;
  if (side == START_BUY)
    ok = trade.Buy(lots, _Symbol, price, 0.0, tp, "Step0 BUY");
  else
    ok = trade.Sell(lots, _Symbol, price, 0.0, tp, "Step0 SELL");

  return ok;
}

// Place the next pending stop based on last triggered entry price and direction
bool PlaceNextPending(double base_entry_price, StartSide next_direction,
                      int next_step) {
  trade.SetExpertMagicNumber(InpMagic);
  ...
  if (next_direction == START_BUY) {
    ...
    return trade.BuyStop(lots, pend_price, _Symbol, 0.0, tp, ORDER_TIME_GTC, 0,
                         "BUY_STOP");
  } else {
    pend_price = base_entry_price - PointsToPrice(InpStep_Points);
    tp = pend_price - PointsToPrice(InpTP_Points);
    return trade.SellStop(lots, pend_price, _Symbol, 0.0, tp, ORDER_TIME_GTC, 0,
                          "SELL_STOP");
  }
}

// Decide what pending should exist based on cycle rules:
// Buy-start cycle: after BUY filled -> place SELL STOP below; after SELL filled
// -> place BUY STOP above; etc. Sell-start cycle: after SELL filled -> place
// BUY STOP below; after BUY filled -> place SELL STOP above; etc.
StartSide NextPendingSideAfterFill(StartSide filled_side,
...
// For price anchoring, always place next pending relative to the entry price of
// the trade that just got triggered.
bool EnsureNextPendingFromLatestFill() {
  ...
  if (next_step >= InpMaxSteps)
    next_step = InpMaxSteps - 1; // cap at last lot
...
void OnTick() {
  ...
  // If we have positions but no pending => place the next pending based on
  // latest fill
  if (CountOurPositions() > 0 && CountOurOrders() == 0) {
...
void OnTradeTransaction(const MqlTradeTransaction &trans,
  ...
  if (reason == DEAL_REASON_TP) {
    ...
    // deal type here is SELL for closing a BUY, BUY for closing a SELL
    // (depends), better read position type from deal's "DEAL_POSITION_ID" but
    // we can infer from profit direction:
