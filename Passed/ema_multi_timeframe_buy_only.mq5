...
void ManagePosition()
{
   ...
   // (1) when profit >= 500 points -> move SL to BE + buffer
   if(profit_pts >= Profit_500_Points)
   {
      ...
      {
         if(ModifyPosition(ticket, be_sl, 0.0))
            Print("Moved SL to BE+", SL_Move_Buffer, " points: ", be_sl);
      }
   }

   // (2) when profit >= 1000 points -> enable trailing by M1 EMA50
