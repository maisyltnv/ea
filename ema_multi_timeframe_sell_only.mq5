...
// Removed Max_Daily_Profit - no longer limiting daily profit
...
void OnTick()
{
   ...
   // Removed daily profit limit check - no longer limiting daily profit
   
   // Check if daily SL count limit reached
   if(daily_sl_count >= Max_Daily_SL_Count)
   {
      trading_allowed_today = false;
      Print("Daily SL limit reached: ", daily_sl_count, " SL hits - No more trading today");
      return;
   }
   ...
   // Additional check: If trading is not allowed today, don't proceed
   if(!trading_allowed_today)
   {
      return;
   }
...
void CheckSellSignal()
{
   ...
   // SELL conditions: price below EMA50 - 50 points AND EMA50 < EMA100 < EMA200
   ...
   // SELL conditions: price below EMA14 - 50 points AND EMA14 < EMA26 < EMA50 < EMA100 < EMA200
...
void OpenSellOrder()
{
   ...
   double raw_sl = swing_high + SL_Buffer_Points*_Point; // SL above swing high
...
void ManagePosition()
{
   ...
   // Check if position is profitable and close to TP
   double open_price = PositionGetDouble(POSITION_PRICE_OPEN);
   double current_price = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
   double profit_points = (open_price - current_price) / _Point;
   
   // If we're very close to TP (within 50 points), stop trading to prevent new entries
   if(profit_points >= (TP_Points - 50))
   {
      if(trading_allowed_today)
      {
         trading_allowed_today = false;
         Print("Position very close to TP (", profit_points, " points). Stopping trading to prevent new entries.");
      }
   }
...
void CheckNewDay()
{
   ...
   if(current_date != last_trade_date)
   {
      ...
      Print("All limits reset - Ready to trade again!");
   }
}
                       ...
                       const MqlTradeResult& result)
{
   ...
   if(trans.type == TRADE_TRANSACTION_DEAL_ADD)
   {
      if(HistoryDealSelect(trans.deal))
      {
         ...
         // Only process our EA's deals
         if(magic == Magic_Number && deal_entry == DEAL_ENTRY_OUT)
         {
            Print("Position closed. Profit: ", profit, " Reason: ", deal_reason, " Magic: ", magic);
            ...
            if(profit < 0)
            {
               ...
               {
                  trading_allowed_today = false;
                  Print("Trading stopped immediately for today due to SL limit reached: ", daily_sl_count, " SL hits");
                  Print("No more trading today, even if conditions are met. Will resume tomorrow.");
               }
            }
            // Check if it was closed by TP - stop trading immediately for the day
            else if(profit > 0)
            {
               trading_allowed_today = false;
               Print("TP hit! Trading stopped immediately for today due to TP reached. Profit: ", profit);
               Print("No more trading today, even if conditions are met. Will resume tomorrow.");
            }
         }
      }
