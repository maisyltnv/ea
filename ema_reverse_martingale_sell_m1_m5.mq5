...
input ulong   InpMagicNumber          = 90010503;   // Magic Number (reverse martingale SELL EA)
...
void TryOpenSell()
{
   ...
   if(InpSlPoints > 0) sl = bid + InpSlPoints * pt; // SL above entry for sell
   if(InpTpPoints > 0) tp = bid - InpTpPoints * pt; // TP below entry for sell
...
{
   trade.SetExpertMagicNumber(InpMagicNumber);
   gvRevMartiStepName = StringFormat("EA_RevMartiStepSell_%s_%I64u", _Symbol, InpMagicNumber);
   if(!GlobalVariableCheck(gvRevMartiStepName))
      GlobalVariableSet(gvRevMartiStepName, 1.0);
